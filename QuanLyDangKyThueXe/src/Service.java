...
//	1. Liệt kê những dòng xe có số chỗ ngồi trên 5 chỗ
	public void lietKeDongXeSoChoNgoiTren(int soCho) {
		System.out.println("Những dòng xe có số chỗ ngồi trên "+soCho+" chỗ: ");
		String query = "SELECT DongXe FROM thue_xe_cms.DONGXE WHERE SoChoNgoi > "+soCho;
...
//	thuộc hãng xe “KIA” với mức phí có đơn giá là 20.000 VNĐ/km
	public void LietKeNCCDieuKienHangXeMucPhi(String hangXe, String mucGia) {
		System.out.println("Các nhà cung cấp ĐÃ TỪNG đăng ký cung cấp những dòng xe " + 
				"thuộc hãng xe “"+hangXe+"” với mức phí có đơn giá là "+mucGia+" VNĐ/km:");
...
//						"                       OR (HangXe = \"KIA\" AND DonGia = \"20000\"))";
						"                       WHERE HangXe = \""+hangXe+"\" AND DonGia = \""+mucGia+"\")";
...
//	cấp và giảm dần theo mã số thuế
	public void LietKeNCC(String tenTruong, boolean tangDan) {
		System.out.println("Thông tin các nhà cung cấp được sắp xếp "+(tangDan ? "tăng" : "giảm")+" dần theo "+tenTruong+": ");
		String query = "SELECT * FROM thue_xe_cms.NHACUNGCAP ORDER BY " + tenTruong +" "+ (tangDan ? "ASC" : "DESC");
...
//	cung cấp là “20/11/2015”
	...
	public void DemSoLanDangKyTheoNCCDieuKienNgayBatDau(Date startDate) {
		System.out.println("Số lần đăng ký cung cấp phương tiện tương ứng cho từng nhà cung cấp (" + 
				"ngày bắt đầu là "+startDate.getDate()+"/"+startDate.getMonth()+"/"+startDate.getYear()+"): ");
		String query = "SELECT MaNhaCC, COUNT(*) FROM thue_xe_cms.DANGKYCUNGCAP WHERE NgayBatDauCungCap = '"+startDate.getYear()+"-"+startDate.getMonth()+"-"+startDate.getDate()+"' GROUP BY MaNhaCC";
//		System.out.println(query);
		ResultSet data = executeQuery(query);
		try {
			while(data.next()) {
...
//	thuộc dòng xe “Hiace” hoặc từng đăng ký cung cấp phương tiện thuộc dòng xe “Cerato”
	public void LietKeNCCDongXe(String...strings) {
		System.out.print("Các nhà cung cấp đã từng đăng ký cung cấp phương tiện " + 
				"thuộc dòng xe ");
		String listDongXe = "";
		...
		listDongXe = listDongXe.substring(0, listDongXe.length()-1);
		System.out.println(listDongXe+": ");
		String query = "SELECT * FROM thue_xe_cms.NHACUNGCAP WHERE MaNhaCC IN" + 
				"		( SELECT MaNhaCC FROM thue_xe_cms.DANGKYCUNGCAP WHERE DongXe IN ("+listDongXe+"))";
//		System.out.println(query);
		NhaCungCap.displayAllInfo(queryGetNCC(query));
	}
